...
// ============ AI Setup ============
let genAI, gayAiModel, normalAiModel;
if (GEMINI_KEY) {
...
function rebuildAIModels() {
    if (!genAI) return;
    gayAiModel = genAI.getGenerativeModel({
        model: "gemini-2.5-flash",
        systemInstruction: GAY_SYSTEM_PROMPT,
    });
    normalAiModel = genAI.getGenerativeModel({
        model: "gemini-2.5-flash",
        systemInstruction: NORMAL_SYSTEM_PROMPT,
    });
}
...
async function askAI(userMessage, userName, userId, serious = !gayMode) {
    const selectedModel = serious ? normalAiModel : gayAiModel;
    ...
    try {
        const result = await selectedModel.generateContent(`${userName}说：${userMessage}`);
        const response = result.response.text();
        const suffix = serious ? "..." : "...😘💕";
...
client.on('messageCreate', async message => {
    ...
    if (message.mentions.has(client.user)) {
        ...
        if ((gayAiModel || normalAiModel) && mentionContent.length > 0) {
    ...
    try {
        switch (commandName) {
            ...
            case 'gaymode': {
                ...
                gayMode = toggle;
                // AI models are already initialized for both modes, so no need to rebuild
            ...
            // ---- /ask ----
            case 'ask': {
                const serious = interaction.options.getBoolean('serious_mode');
                const question = interaction.options.getString('question');
                if (!gayAiModel || !normalAiModel) {
                    return interaction.reply({ content: gayMode ? "Sky的AI大脑还没装上呢...让主人设置GEMINI_API_KEY先😘" : "AI功能未启用，需要设置GEMINI_API_KEY。", ephemeral: true });
                }
                await interaction.deferReply();
                const aiReply = await askAI(question, interaction.user.username, interaction.user.id, serious);
                if (aiReply) {
                    await interaction.editReply(`💬 **${interaction.user.username}问：** ${question}\n\n${aiReply}`);
                } else {
                    await interaction.editReply((!serious)
